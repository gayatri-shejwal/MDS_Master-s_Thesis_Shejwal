...
levels(long_subset_df$age_group) = c("21-30", "31-40", "41-50", "51-60")
levels(long_subset_df$educ) = c("Bachelor's", "Master's", "PhD")
levels(long_subset_df$ml) = c("ml_None", "ml_Basic", 
                                   "ml_Intermediate", "ml_Advanced")
levels(long_subset_df$prog) = c("prog_None", "prog_Basic", 
                                   "prog_Intermediate", "prog_Advanced")
levels(long_subset_df$soft) = c("soft_Low", "soft_Medium", "soft_High")
levels(long_subset_df$lead) = c("lead_Low", "lead_Medium", "lead_High")
levels(long_subset_df$ai_framing) = c("Low", "High")
levels(long_subset_df$job_role) = c("Manager", "Analyst")
...
for (attribute in c("age_group", "educ", "ml", "prog", "soft", "lead")) {
    ...
    data_model_c <- long_subset_df[, c(attribute, "chosen")]
    ...
    vcov_c         <- vcovCluster(model_c, long_subset_df$RespID)
...
# Subset by job role
manager_data <- subset(long_subset_df, job_role == "Manager")
...
for (attribute in c("age_group", "educ", "ml", "prog", "soft", "lead")) {
    ...
    # Create data frame for model
    data_model_manager <- long_subset_df[, c(attribute, "chosen")]
    ...
    vcov_manager         <- vcovCluster(model_manager, long_subset_df$RespID)
...
analyst_data <- subset(long_subset_df, job_role == "Analyst")
...
for (attribute in c("age_group", "educ", "ml", "prog", "soft", "lead")) {
    ...
    # Create data frame for model
    data_model_analyst <- long_subset_df[, c(attribute, "chosen")]
    ...
    vcov_analyst         <- vcovCluster(model_analyst, long_subset_df$RespID)
...
long_subset_df_low <- subset(long_subset_df, ai_framing == "Low")
long_subset_df_high <- subset(long_subset_df, ai_framing == "High")

conjacp_data_cjoint_low <- conjacp.prepdata(
  chosen ~ age_group + educ + ml*prog + soft + lead,
  data = long_subset_df_low,
  tasks = "task_fg",
  #subgroups = "ai_framing",
  id = "RespID"
)
